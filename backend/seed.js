...
async function seed() {
  ...
  for (const c of CARS) {
    const [carRes] = await pool.query(
      'INSERT INTO cars (title, make, model, year, mileage, price, currency, body_type, location, latitude, longitude, `condition`, description, photos, featured, seller_id) VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)',
      [c.title, c.make, c.model, c.year, c.mileage, c.price, c.currency, c.bodyType, c.location, c.latitude, c.longitude, c.condition, c.description, JSON.stringify(c.photos), c.featured, sellerId]
    );
